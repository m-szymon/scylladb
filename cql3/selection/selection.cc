...
static bool contains_collection_mutation_attribute(const expr::expression& e) {
    return expr::find_in_expression<expr::column_mutation_attribute>(e, [](const expr::column_mutation_attribute& cma) {
        return expr::is<expr::subscript>(cma.column);
    });
}
...
void result_set_builder::add_collection(const column_definition& def, bytes_view c) {
    ...
    if (!_collection_element_metadata.empty() && c.size() >= sizeof(int32_t)) {
        // Peek at the first 4 bytes to see if we have the extended format (magic -1).
        bytes_view peek = c;
        int32_t magic = read_simple<int32_t>(peek);
        if (magic == int32_t(-1)) {
            // Extended format: [magic=-1][uint32 cql_len][cql bytes][int32 entry_count][entries...]
            uint32_t cql_len = read_simple<uint32_t>(peek);
            bytes_view cql_bytes = read_simple_bytes(peek, cql_len);
            ...
            int32_t entry_count = read_simple<int32_t>(peek);
            for (int32_t i = 0; i < entry_count; ++i) {
                int32_t key_len = read_simple<int32_t>(peek);
                bytes key = to_bytes(read_simple_bytes(peek, key_len));
                int64_t ts = read_simple<int64_t>(peek);
                int64_t expiry_raw = read_simple<int64_t>(peek);
                meta.timestamps[key] = ts;
                if (expiry_raw != -1) {
                    auto expiry = gc_clock::time_point(gc_clock::duration(expiry_raw));
                    auto ttl_left = expiry - _now;
                    meta.ttls[key] = std::max(int32_t(0), int32_t(ttl_left.count()));
                } else {
                    meta.ttls[key] = -1;
                }
            }
        ...
        // Not extended format - clear stale metadata for this slot
        _collection_element_metadata[col_idx] = {};
    }
