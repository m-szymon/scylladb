...
# that does not exist on DynamoDB, so all tests in this file are skipped
...
from .util import random_string, new_test_table, unique_table_name, scylla_config_read, scylla_config_write
...
def test_numeric_list_precision_range(dynamodb, test_table_s):
    p = random_string()
...
# rejected. When VectorSearch isn't specified, our code only looks for
# a base table or LSI or GSI, so if IndexName points to a vector index
# it won't be found, resulting in a ValidationException.
#
# FIXME: this behavior is a bit weird, and we may want to change it in the
# future - we may want to recognize that IndexName points to a vector index,
# and then give a more specific error message that VectorSearch is required
# for vector indexes.
def test_query_vectorindex_no_vectorsearch(table_vs):
    with pytest.raises(ClientError, match='ValidationException.*vind'):
        table_vs.query(
            IndexName='vind',
...
def unconfigured_vector_store(vs):
    ...
    # The following doesn't work due to #28225... Hopefully someday it will work.
    #with scylla_config_temporary_string(vs, 'vector_store_primary_uri', 'garbage'):
    #    yield
...
def test_vector_with_ttl(vs, needs_vector_store, have_ck):
    ...
    with new_test_table(vs,
        ...
        while True:
            ...
            time.sleep(0.1)
        # Seeing no results above does NOT prove the vector store noticed the
        # deletion. It is possible that the vector store still has the item
        # but the Query with (by default) Select=ALL_ATTRIBUTES looks for the
        # returned keys in the base table, and because those are gone (TTL
        # deleted them), the Query returns no rows.
        # So let's run the search again with Select='ALL_PROJECTED_ATTRIBUTES'
        # to verify that the results are really gone from the vector index
        # and not just filtered out in Scylla.
        # We need to do a retry loop again - the previous loop ended when the
        # items disappeared from the base-table, but the vector store can
        # still return the outdated results for a while longer and we see them
        # when we use ALL_PROJECTED_ATTRIBUTES.
        deadline = time.monotonic() + VECTOR_STORE_TIMEOUT + float(period)
        while True:
            result = table.query(IndexName='vind',
                VectorSearch={'QueryVector': [1, 0, 0]},
            ...
            if result['Items'] == []:
                break
            if time.monotonic() > deadline:
                pytest.fail('Timed out waiting for TTL-expired item to disappear from vector search with ALL_PROJECTED_ATTRIBUTES')
            time.sleep(0.1)
        assert result['Items'] == []
...
def test_query_vectorsearch_select(vs, needs_vector_store):
    with new_test_table(vs,
        ...
        while True:
            ...
            time.sleep(0.1)
        # ALL_ATTRIBUTES (default when no Select): returns the full item.
        result = table.query(IndexName='vind',
            VectorSearch={'QueryVector': [1, 0, 0]}, Limit=1)
        assert result['Items'] == [{'p': p, 'v': [1, 0, 0], 'x': 'hello', 'y': 'world'}]
        # Explicit Select=ALL_ATTRIBUTES: same result.
        result = table.query(IndexName='vind',
            VectorSearch={'QueryVector': [1, 0, 0]}, Limit=1,
            Select='ALL_ATTRIBUTES')
        assert result['Items'] == [{'p': p, 'v': [1, 0, 0], 'x': 'hello', 'y': 'world'}]
        # Select=ALL_PROJECTED_ATTRIBUTES: returns only primary key attributes.
        result = table.query(IndexName='vind',
            VectorSearch={'QueryVector': [1, 0, 0]}, Limit=1,
            Select='ALL_PROJECTED_ATTRIBUTES')
