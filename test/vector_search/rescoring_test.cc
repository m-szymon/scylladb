...
int get_id_col_value(const auto& row) {
    constexpr auto ID_COL_INDEX = 0;
    return value_cast<int>(int32_type->deserialize(row.at(ID_COL_INDEX).value()));
}

float get_similarity_col_value(const auto& row) {
    constexpr auto SIMILARITY_COL_INDEX = 1;
    return value_cast<float>(float_type->deserialize(row.at(SIMILARITY_COL_INDEX).value()));
}
        ...
        co_await do_with_cql_env(
                [&](cql_test_env& env) -> future<> {
                    ...
                    auto msg = co_await env.execute_cql(fmt::format(
                            "SELECT id, similarity_{}(embedding, [0.1, 0.1]) FROM ks.cf ORDER BY embedding ANN OF [0.1, 0.1] LIMIT 2;", params.function_name));
