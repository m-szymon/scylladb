        ...
        for (const index_metadata& im : schema->indices()) {
            ...
            if (dims_it != opts.end()) {
                try {
                    ...
                    elogger.error("Unexpected non-integer dimensions value '{}' for vector index '{}'", dims_it->second, im.name());
                }
            }
...
future<executor::request_return_type> executor::create_table_on_shard0(service::client_state&& client_state, tracing::trace_state_ptr trace_state, rjson::value request, bool enforce_authorization, bool warn_authorization, const db::tablets_mode_t::mode tablets_mode) {
    ...
    // happens before we actually creating the table, so if we have a parse
    ...
    if (vector_indexes) {
        ...
        for (const rjson::value& v : vector_indexes->GetArray()) {
            ...
            if (!dimensions_v || !dimensions_v->IsInt() || dimensions_v->GetInt() <= 0 || (vector_dimension_t)dimensions_v->GetInt() > cql3::cql3_type::MAX_VECTOR_DIMENSION) {
                co_return api_error::validation(fmt::format("VectorIndexes Dimensions must be an integer between 1 and {}.", cql3::cql3_type::MAX_VECTOR_DIMENSION));
            }
...
static future<executor::request_return_type> query_vector(
    ...
    auto pkeys_result = co_await vsc.ann(
            ...
            std::move(query_vec), limit, rjson::empty_object(), aoe.abort_source());
...
future<executor::request_return_type> executor::query(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    if (rjson::find(request, "VectorSearch")) {
        ...
        return query_vector(_proxy, _vsc, request, client_state, trace_state, std::move(permit),
