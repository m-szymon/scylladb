...
1. **Parse and prepare.** The ordering can be written either as `column ANN OF vector` or as the equivalent function call `ANN(column, vector)`; the grammar lowers the former to the latter, so everything downstream sees one representation. The CQL grammar accepts ANN ordering only as a single `ORDER BY` item. Descending order and combined orderings are rejected. ScyllaDB validates that the ordered column is `vector<float, N>` with an existing `vector_index`. Requirements: `LIMIT` is mandatory; `PER PARTITION LIMIT` and aggregation are forbidden; `LIMIT` above 1000 is rejected. The mandatory limit exists because HNSW graph traversal cannot be meaningfully paged — the search must complete in a single pass to produce correctly ranked results — and in practice vector similarity search use cases rarely need more than a 1000 top-k result set.
...
4. **Receive candidate keys.** The Vector Store responds with `primary_keys` and `similarity_scores`. ScyllaDB deserializes the keys into decorated partition keys plus clustering keys. What the Vector Store said about a row - the similarity it reported and the rank that similarity put the row at - reaches the client where the query selects `ANN()`, which answers with the pair, or `ANN_SCORE()`/`ANN_RANK()`, which name the halves. Each reading has one temporary slot, allocated on the first occurrence that asks for it and filled per row by a provider walking the response; every occurrence of a reading shares its slot, since all of them are required to name the same search. With rescoring enabled the score comes from the local recomputation instead, and the rank is reported as `0` - see step 7. When rescoring is disabled, ScyllaDB preserves the Vector Store's ordering. When rescoring is enabled, ScyllaDB recomputes exact similarity from base-table vectors and reorders based on its own scores.
...
7. **Rescore (optional).** Rescoring is enabled when `quantization != f32` **and** `rescoring = "true"`. During prepare, ScyllaDB injects a hidden similarity selector (`cosine`, `euclidean`, or `dot_product`). After fetching oversampled rows, it computes exact similarity from the stored full-precision vectors, sorts by score, trims to the CQL limit, and hides the score column from the client. A query that selects the score gets that same computation once more per occurrence, in the position it asked for; sharing the hidden selector instead is left for later. The rank is reported as `0`: the rows are no longer in the order the Vector Store ranked them, so its rank would be a wrong answer, and the rank in the recomputed order needs every row's score at once, which this path does not hold. `0` is outside the range a rank occupies, so it reads as "not reported" rather than as the null that means "this search did not find the row". Delivering a real rank means moving the recomputation to where the whole result is in hand.
