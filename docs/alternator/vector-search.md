...
|----------------|----------|
| `ALL_ATTRIBUTES` (default) | Return all attributes of each matching item, fetched from the base table. |
| `ALL_PROJECTED_ATTRIBUTES` | Return only the attributes projected to the vector index. Currently, only the primary key attributes (hash key, and range key if present) are projected; support for configuring additional projected attributes is not yet implemented. This option is more efficient than `ALL_ATTRIBUTES` because Scylla can return the results directly from the vector store without an additional fetch from the base table. |
...
`Select` defaults to `ALL_ATTRIBUTES`.  When `ProjectionExpression` or
...
- `Select=ALL_ATTRIBUTES` (default): Each candidate item is fetched from the
...
- `Select=ALL_PROJECTED_ATTRIBUTES`: When no filter is present this is the most
